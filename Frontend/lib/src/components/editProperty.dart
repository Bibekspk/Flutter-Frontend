...
class _EditRoomPageState extends State<EditRoom> {
  ...
  void sendregdata() async {
    ...
    var response = await http.put(url,
        ...
        body: json.encode(editRoom.toJson()));
    print(response.body);
  ...
  Future singleUpload() async {
    ...
    try {
      ...
      // Dio dio = new Dio();
      var response =
          await dio.put("http://10.0.2.2:5000/v2/editsingleupload/$roomId",
              data: formData,
              options: Options(
      ...
      if (images != null) {
        ...
        for (var i = 0; i < images.length; i++) {
          ...
          var response = await dio.put(
              "http://10.0.2.2:5000/v2/$userId/$roomId/editmultipleuploads",
              data: formData,
              options: Options(
                headers: {"authorization": "$token"},
              ));
